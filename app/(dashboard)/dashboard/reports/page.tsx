  ...
  return (
    <div style={{direction:'rtl',fontFamily:'system-ui'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:14,marginBottom:24}}>
        {[
          { label:'إجمالي المصروفات', value: totalIncl.toFixed(2)+' ﷼', color:'#6366f1', bg:'#eef2ff', border:'#c7d2fe' },
          { label:'إجمالي الضريبة',   value: totalVat.toFixed(2)+' ﷼',  color:'#f59e0b', bg:'#fffbeb', border:'#fde68a' },
          { label:'قبل الضريبة',      value: totalNoVat.toFixed(2)+' ﷼', color:'#0891b2', bg:'#ecfeff', border:'#a5f3fc' },
          { label:'عدد الفواتير',     value: filtered.length,             color:'#16a34a', bg:'#f0fdf4', border:'#bbf7d0' },
          { label:'بضريبة',           value: withVat,                     color:'#f59e0b', bg:'#fffbeb', border:'#fde68a' },
          { label:'بدون ضريبة',       value: noVat,                       color:'#64748b', bg:'#f8fafc', border:'#e2e8f0' },
        ].map((s,i) => (
              ...
              <tbody>
                {filtered.map((p,i) => (
                  <tr key={i} style={{borderBottom:'1px solid #f1f5f9',background:i%2===0?'white':'#fafafa'}}>
                    ...
                    <td style={{padding:'12px 14px',textAlign:'right'}}><div style={{fontWeight:700,fontSize:14,color:'#0f172a'}}>{p.product_name||'—'}</div></td>
                    ...
                    <td style={{padding:'12px 14px',textAlign:'center'}}>
                      <span style={{padding:'4px 10px',borderRadius:50,fontSize:11,fontWeight:700,
