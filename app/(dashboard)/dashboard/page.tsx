    ...
    <div style={{direction:'rtl',fontFamily:'system-ui',maxWidth:1100,margin:'0 auto'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:16,marginBottom:28}}>
        {[
          ...
          {label:'إجمالي المشتريات',icon:'🧾',value:totalSpent.toLocaleString('ar-SA',{maximumFractionDigits:0})+' ﷼',sub:`${purchases.length} عملية شراء`,color:'#0891b2',bg:'linear-gradient(135deg,#ecfeff,#cffafe)',border:'#a5f3fc'},
          {label:'أصناف ناقصة',icon:'⚠️',value:lowStock.length,sub:'تحتاج طلب عاجل',color:'#dc2626',bg:'linear-gradient(135deg,#fef2f2,#fee2e2)',border:'#fecaca'},
          ...
          <div key={i} style={{background:s.bg,border:`1.5px solid ${s.border}`,borderRadius:18,padding:'22px 24px',boxShadow:'0 2px 8px rgba(0,0,0,0.05)'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
        ...
        <div style={{background:'white',borderRadius:18,overflow:'hidden',boxShadow:'0 2px 12px rgba(0,0,0,0.06)'}}>
          ...
          <div style={{padding:'8px 0'}}>
            {purchases.length === 0 ? (
              <div style={{textAlign:'center',padding:'32px 20px',color:'#94a3b8',fontSize:13,fontWeight:600}}>لا توجد مشتريات بعد</div>
            ) : purchases.map((p,i) => (
        ...
        <div style={{background:'white',borderRadius:18,overflow:'hidden',boxShadow:'0 2px 12px rgba(0,0,0,0.06)'}}>
          <div style={{padding:'18px 22px',borderBottom:'1px solid #f1f5f9',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <h3 style={{fontSize:15,fontWeight:800,color:'#0f172a',margin:0}}>📤 آخر عمليات الصرف</h3>
          ...
          <div style={{padding:'8px 0'}}>
            {dispenses.length === 0 ? (
              <div style={{textAlign:'center',padding:'32px 20px',color:'#94a3b8',fontSize:13,fontWeight:600}}>لا توجد عمليات صرف بعد</div>
            ) : dispenses.map((d,i) => (
