...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const mainLinks = [
    { href:'/dashboard',           label: tx.dashboard,  emoji:'📊' },
    { href:'/dashboard/inventory', label: tx.inventory,  emoji:'📦' },
    { href:'/dashboard/purchases', label: tx.purchases,  emoji:'🛒' },
    { href:'/dashboard/dispenses', label: tx.dispenses,  emoji:'📤' },
    { href:'/dashboard/settings',  label: tx.settings,   emoji:'⚙️' },
  ]
  ...
  return (
    <div style={{
          ...
          <div style={{padding:'14px 14px 10px', borderBottom:'1px solid #f3f4f6'}}>
            <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10}}>
              ...
              <div style={{display:'flex', alignItems:'center', gap:8}}>
                <div style={{width:26, height:26, background:'linear-gradient(135deg,#1d4ed8,#6366f1)', borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12}}>🏪</div>
                <span style={{fontSize:14, fontWeight:700, color:'#111827'}}>Storely</span>
              </div>
            </div>
            <div style={{background:'linear-gradient(135deg,#eff6ff,#eef2ff)', border:'1px solid #dbeafe', borderRadius:10, padding:'10px 12px', display:'flex', alignItems:'center', gap:10}}>
              <div style={{width:32, height:32, flexShrink:0, background:'linear-gradient(135deg,#1d4ed8,#6366f1)', borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, color:'white', fontWeight:700}}>
                {orgName?.[0]?.toUpperCase() || '؟'}
              </div>
              <div style={{minWidth:0}}>
                <div style={{fontSize:13, fontWeight:700, color:'#1e293b', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{orgName || '...'}</div>
                <div style={{fontSize:10, color:'#6366f1', fontWeight:500, marginTop:1}}>{tx.basicPlan}</div>
              </div>
            </div>
          </div>

          <nav style={{flex:1, padding:'8px', overflowY:'auto'}}>
            <div style={{fontSize:9, fontWeight:600, color:'#9ca3af', padding:'6px 12px 4px', letterSpacing:'0.1em', textTransform:'uppercase'}}>{tx.menu}</div>
            {mainLinks.map(link => {
              ...
              return (
                <Link key={link.href} href={link.href}
                  ...
                  {link.label}
                  {isActive && <span style={{marginRight: lang==='ar'?'auto':undefined, marginLeft: lang==='en'?'auto':undefined, width:5, height:5, borderRadius:'50%', background:'#1d4ed8', flexShrink:0}} />}
                </Link>
              )
            })}

            <div style={{height:1, background:'#f3f4f6', margin:'6px 12px'}} />
            <div style={{fontSize:9, fontWeight:600, color:'#9ca3af', padding:'4px 12px', letterSpacing:'0.1em', textTransform:'uppercase'}}>{tx.reports}</div>

            <button onClick={() => setReportsOpen(!reportsOpen)} className={`nav-item ${isReportActive ? 'active' : ''}`}>
              <span className="nav-icon">📈</span>
              {tx.reports}
              <span style={{marginRight: lang==='ar'?'auto':undefined, marginLeft: lang==='en'?'auto':undefined, fontSize:10, color:'#9ca3af', transform: reportsOpen?'rotate(180deg)':'none', transition:'transform 0.2s', display:'inline-block'}}>▾</span>
            </button>

            {reportsOpen && (
              <div style={{paddingRight: lang==='ar'?12:0, paddingLeft: lang==='en'?12:0, borderRight: lang==='ar'?'2px solid #e5e7eb':'none', borderLeft: lang==='en'?'2px solid #e5e7eb':'none', marginRight: lang==='ar'?20:0, marginLeft: lang==='en'?20:0, marginBottom:4}}>
      ...
      <div style={{flex:1, minHeight:'100vh', display:'flex', flexDirection:'column'}}>
        <header style={{background:'white', borderBottom:'1px solid #e5e7eb', padding:'0 20px', height:52, display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:40, boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
          ...
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            ...
            <div style={{display:'flex', alignItems:'center', gap:5, padding:'4px 10px', background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:6}}>
            ...
            <div style={{width:30, height:30, background:'linear-gradient(135deg,#1d4ed8,#6366f1)', borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, color:'white', fontWeight:700, cursor:'pointer', boxShadow:'0 2px 8px rgba(29,78,216,0.25)'}}>
