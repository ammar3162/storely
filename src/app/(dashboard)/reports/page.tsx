...
function MobilePurchaseCard({ p }: { p: any }) {
  ...
  return (
    <div style={{ background: '#fafbfc', border: `1px solid ${colors.border}`, borderRadius: 14, padding: 14, marginBottom: 10 }}>
      ...
      {[['بدون ضريبة', Number(p.amount || 0).toFixed(2), colors.text2], ['الضريبة 15%', Number(p.vat_amount || 0).toFixed(2), colors.warning], ['الإجمالي', Number(p.total_amount || 0).toFixed(2), colors.primary]].map(([lbl, val, clr], i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderTop: i === 0 ? `1px dashed ${colors.border}` : 'none', marginTop: i === 0 ? 8 : 0 }}>
          <span style={{ fontSize: 'clamp(0.7rem, 2.6vw, 0.8rem)', color: i === 2 ? colors.primary : colors.text4, fontWeight: i === 2 ? 800 : 600 }}>{lbl}</span>
          <span style={{ fontSize: i === 2 ? 'clamp(0.95rem, 3.5vw, 1.1rem)' : 'clamp(0.85rem, 3.2vw, 0.95rem)', fontWeight: i === 2 ? 900 : 800, color: clr }}>{val} ر.س</span>
        </div>
      ))}
