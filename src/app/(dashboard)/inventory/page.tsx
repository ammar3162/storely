        ...
        {filtered.length===0 ? (
          ...
          <>
            ...
            <div className="mob-cards" style={{padding:'10px'}}>
              <style>{`@media(min-width:640px){.mob-cards{display:none!important}.desk-table{display:block!important}}@media(max-width:639px){.desk-table{display:none!important}}.mob-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}`}</style>
              ...
              {paginated.map((p,i)=>{
                ...
                return (
                  <div key={p.id} onClick={()=>openEdit(p)} style={{background:colors.surface,border:`1px solid ${statusBorder}`,borderRadius:10,padding:'10px 8px',cursor:'pointer'}}>
