  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (form.category==='مخزون' && form.name) {
      ...
      if (existing) {
        ...
        const { data: np } = await sb.from('products').insert({org_id:orgId,branch_id:branchId,name:form.name,sku:form.sku||null,unit:form.unit||'قطعة',qty,reorder_point:Number(form.reorder_point)||5,is_active:true}).select().single()
