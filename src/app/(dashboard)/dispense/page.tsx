    ...
    <div style={{fontFamily:"'IBM Plex Sans Arabic',system-ui",direction:'rtl',maxWidth:960,margin:'0 auto',opacity:visible?1:0,transition:'opacity .3s'}}>
      <style>{`
        *{box-sizing:border-box}
        ...
        .pcard{border-radius:12px;padding:12px 14px;cursor:pointer;border:1.5px solid;background:white;transition:all .15s;text-align:right;font-family:inherit;width:100%;display:flex;align-items:center;gap:12px}
        ...
        @media(min-width:768px){.prod-grid{grid-template-columns:repeat(3,1fr);gap:10px}}
              ...
              {displayed.map(p=>{
                ...
                return (
                  <button key={p.id} className="pcard"
                    ...
                    {/* Qty circle */}
                    <div style={{width:44,height:44,borderRadius:12,background:sb2,border:`1.5px solid ${sbb}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                      <span style={{fontSize:15,fontWeight:900,color:sc}}>{p.qty}</span>
                    </div>
                    {/* Info */}
                    <div style={{flex:1,minWidth:0,textAlign:'right'}}>
                      ...
                      <div style={{fontSize:10,color:C.text4,marginTop:2}}>{p.unit} · حد {p.reorder_point}</div>
                    ...
                    <div style={{flexShrink:0}}>
                      <span style={{fontSize:9,fontWeight:700,color:sc,background:sb2,padding:'3px 8px',borderRadius:99,border:`1px solid ${sbb}`,display:'block',whiteSpace:'nowrap'}}>
