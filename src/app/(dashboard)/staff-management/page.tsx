          ...
          <div style={{background:'white',borderRadius:radius.md,padding:'14px 16px',marginBottom:14,border:`1px solid ${colors.primaryBorder}`}}>
            ...
            <div>
              ...
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                ...
                <button onClick={()=>{navigator.clipboard.writeText(`رابط دخول الموظف: https://storely-hm1u.vercel.app/staff\nرقم الجوال: ${revealedPin.phone}\nرمز PIN: ${revealedPin.pin}`);toast('تم النسخ ✓')}} style={{background:colors.primary,color:'white',border:'none',borderRadius:8,padding:'8px 14px',fontSize:font.xs,fontWeight:700,cursor:'pointer',fontFamily:font.family,whiteSpace:'nowrap' as const}}>
                  📋 نسخ الكل
                </button>
              </div>
            </div>
                ...
                <div style={{marginTop:16,paddingTop:16,borderTop:`1px solid ${colors.border}`,animation:'slideUp .2s ease'}}>
                  ...
                  <div>
                    ...
                    <div style={{display:'flex',gap:8,alignItems:'center'}}>
                      ...
                      <button onClick={()=>{navigator.clipboard.writeText(`رابط دخول الموظف: https://storely-hm1u.vercel.app/staff\nرقم الجوال: ${s.phone}\nرمز PIN: ${s.pin}`);toast('تم النسخ ✓')}} style={{background:colors.primary,color:'white',border:'none',borderRadius:8,padding:'9px 14px',fontSize:font.xs,fontWeight:700,cursor:'pointer',fontFamily:font.family,whiteSpace:'nowrap' as const}}>
                        📋 نسخ الكل
                      </button>
                    </div>
                  </div>
