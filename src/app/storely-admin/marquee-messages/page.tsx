...
export default function MarqueeMessagesPage() {
  ...
  async function addMessage() {
    if (!text.trim()) { setMsg('اكتب نص الرسالة'); return }
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) { setText(''); setMsg('✅ تمت الإضافة'); load() }
    ...
    <div style={{background:C.bg, minHeight:'100vh', padding:32, fontFamily:'system-ui', direction:'rtl'}}>
      <div style={{maxWidth:800, margin:'0 auto'}}>
        ...
        <div style={{background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20, marginBottom:24}}>
          <h2 style={{color:C.text, fontSize:15, fontWeight:700, marginBottom:14}}>إضافة رسالة جديدة</h2>
          {msg && <div style={{color:msg.startsWith('✅')?C.green:C.red, fontSize:13, marginBottom:12}}>{msg}</div>}
          <input placeholder="نص الرسالة" value={text} onChange={e=>setText(e.target.value)}
            style={{width:'100%', padding:'10px 12px', borderRadius:8, border:`1px solid ${C.border}`, background:C.bg, color:C.text, fontSize:14, marginBottom:14, boxSizing:'border-box' as const}}/>
          <button onClick={addMessage} disabled={saving}
        ...
        <div style={{background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20}}>
          ...
          {loading ? <div style={{color:C.text3}}>جاري التحميل...</div> :
            ...
            <div style={{display:'flex', flexDirection:'column', gap:10}}>
              {messages.map((m:any) => (
                <div key={m.id} style={{display:'flex', alignItems:'center', gap:12, padding:'10px 12px', background:C.bg, borderRadius:8, opacity:m.is_active?1:.5}}>
