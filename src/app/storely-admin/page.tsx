...
const STATUS: Record<string,{label:string;color:string;bg:string;dot:string}> = {
  ...
  active:    {label:'مفعّل',   color:'#115e59', bg:'#ccfbf1', dot:'#029FA2'},
...
export default function AdminPage() {
  ...
  const [setup2FAMsg, setSetup2FAMsg] = useState('')
  const [setup2FALoading, setSetup2FALoading] = useState(false)

  async function start2FASetup() {
    setSetup2FAOpen(true); setSetup2FAMsg(''); setSetup2FACode('')
    const res = await fetch('/api/admin/2fa-setup', { method:'POST', headers:{'x-admin-key':sessionStorage.getItem('storely_admin_pass')||''} })
    const data = await res.json()
    if (data.qrDataUrl) setSetup2FAQr(data.qrDataUrl)
  }

  async function confirm2FASetup() {
    setSetup2FALoading(true)
    const res = await fetch('/api/admin/2fa-confirm', { method:'POST', headers:{'Content-Type':'application/json','x-admin-key':sessionStorage.getItem('storely_admin_pass')||''}, body: JSON.stringify({ code: setup2FACode }) })
    const data = await res.json()
    setSetup2FALoading(false)
    if (data.success) {
      setSetup2FAOpen(false)
      const updated = { ...(currentAdmin as any), totp_enabled: true }
      setCurrentAdmin(updated as any)
      sessionStorage.setItem('storely_admin_info', JSON.stringify(updated))
      setSetup2FAMsg('✅ تم تفعيل التحقق بخطوتين بنجاح')
    } else {
      setSetup2FAMsg(`❌ ${data.error || 'خطأ غير متوقع'}`)
    }
  }

  async function disable2FA() {
    const code = prompt('أدخل الرمز الحالي من التطبيق لتأكيد إلغاء التحقق بخطوتين')
    if (!code) return
    const res = await fetch('/api/admin/2fa-disable', { method:'POST', headers:{'Content-Type':'application/json','x-admin-key':sessionStorage.getItem('storely_admin_pass')||''}, body: JSON.stringify({ code }) })
    const data = await res.json()
    if (data.success) {
      const updated = { ...(currentAdmin as any), totp_enabled: false }
      setCurrentAdmin(updated as any)
      sessionStorage.setItem('storely_admin_info', JSON.stringify(updated))
      alert('تم إلغاء التحقق بخطوتين')
    } else {
  ...
  async function login(e: React.FormEvent) {
    ...
    const data = await res.json()
    if (res.ok && data.success && data.needs2FA) {
  ...
  async function verify2FALogin(e: React.FormEvent) {
    e.preventDefault()
    setTwoFAErr(false)
    const res = await fetch('/api/admin/2fa-login-verify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pendingToken,code:twoFACode})})
    const data = await res.json()
    if (res.ok && data.success) {
      finalizeLogin(data)
    } else { setTwoFAErr(true); setTimeout(()=>setTwoFAErr(false),2000) }
  }
  ...
  if (!authed) return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#f8fafc 0%,#f1f5f9 100%)',fontFamily:"'IBM Plex Sans Arabic',system-ui,sans-serif",direction:'rtl',padding:20}}>
      ...
      <div style={{background:'#ffffff',borderRadius:24,padding:'44px 40px',width:'100%',maxWidth:400,boxShadow:'0 24px 60px rgba(15,23,42,.1)',border:'1px solid #f1f5f9',animation:'fadeUp .4s ease',position:'relative',zIndex:1,textAlign:'center'}}>
        <div style={{width:72,height:72,background:'linear-gradient(135deg,#029FA2,#0f766e)',borderRadius:20,display:'flex',alignItems:'center',justifyContent:'center',fontSize:32,margin:'0 auto 20px',boxShadow:'0 8px 24px rgba(22,163,74,.25)'}}>🔐</div>
        ...
        {needs2FA ? (
          <form onSubmit={verify2FALogin}>
            <div style={{fontSize:13,color:'#64748b',marginBottom:14,lineHeight:1.7}}>افتح تطبيق المصادقة (Google Authenticator) وأدخل الرمز المكوّن من 6 أرقام</div>
            <input type="text" inputMode="numeric" maxLength={6} required value={twoFACode} onChange={e=>setTwoFACode(e.target.value.replace(/\D/g,''))} dir="ltr" autoFocus
            ...
            <button type="submit" style={{width:'100%',padding:'15px',background:'linear-gradient(135deg,#029FA2,#0f766e)',color:'white',border:'none',borderRadius:14,fontSize:16,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow:'0 6px 20px rgba(22,163,74,.3)',transition:'all .2s',marginBottom:10}}>
              تأكيد ←
            </button>
        ...
        <form onSubmit={login}>
          ...
          <button type="submit" style={{width:'100%',padding:'15px',background:'linear-gradient(135deg,#029FA2,#0f766e)',color:'white',border:'none',borderRadius:14,fontSize:16,fontWeight:800,cursor:'pointer',fontFamily:'inherit',boxShadow:'0 6px 20px rgba(22,163,74,.3)',transition:'all .2s'}}>
            دخول ←
          </button>
        </form>
        )}
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f8fafc',direction:'rtl',fontFamily:"'IBM Plex Sans Arabic',system-ui,sans-serif",color:'#0f172a'}}>
      <style>{`
        ...
        .filt-btn.act{background:#029FA2;color:white;border-color:#029FA2}
        ...
        .action-btn:hover{border-color:#029FA2;color:#029FA2;background:#f0fdfa}
        input:focus{outline:none!important;border-color:#029FA2!important;box-shadow:0 0 0 3px rgba(22,163,74,.1)!important}
        ...
        <div style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
          ...
          <div style={{background:'#ffffff',border:'1px solid #e5e7eb',borderRadius:24,width:'100%',maxWidth:480,position:'relative',animation:'modalIn .2s ease',boxShadow:'0 32px 80px rgba(15,23,42,.2)',overflow:'hidden',maxHeight:'90vh',overflowY:'auto'}}>
            ...
            <div style={{background:'#f8fafc',padding:'22px 24px',display:'flex',alignItems:'center',gap:14,borderBottom:'1px solid #e5e7eb',position:'sticky',top:0,zIndex:10}}>
              <div style={{width:48,height:48,borderRadius:14,background:'linear-gradient(135deg,#029FA2,#0f766e)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,fontWeight:900,color:'white',flexShrink:0}}>
            ...
            <div style={{padding:'20px 24px'}}>
              ...
              <div style={{display:'flex',gap:8,marginBottom:20,flexWrap:'wrap'}}>
                ...
                {(() => { const d = daysLeft(selected.subscription_ends_at); return d !== null ? (
                  <span style={{background:d<=0?'#fef2f2':d<=3?'#fffbeb':'#f0fdfa',color:d<=0?'#dc2626':d<=3?'#d97706':'#029FA2',padding:'5px 12px',borderRadius:20,fontSize:12,fontWeight:700}}>
                ...
                <div style={{background:'#f0fdfa',border:'1px solid #99f6e4',borderRadius:14,padding:16,marginBottom:16}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6,marginBottom:12}}>
                    {[7,30,90,365].map(d=>(
                      <button key={d} onClick={()=>setRenewDays(d)}
                        style={{padding:'9px 4px',borderRadius:9,border:`1.5px solid ${renewDays===d?'#029FA2':'#e5e7eb'}`,background:renewDays===d?'#029FA2':'#ffffff',color:renewDays===d?'white':'#64748b',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit',transition:'all .15s'}}>
                  ...
                  <div style={{display:'flex',gap:8}}>
                    ...
                    <button onClick={()=>activate(selected.id,'paid',renewDays)} disabled={!!saving}
                      style={{flex:2,padding:'11px',background:'linear-gradient(135deg,#029FA2,#0f766e)',color:'white',border:'none',borderRadius:10,fontSize:13,fontWeight:700,cursor:'pointer',fontFamily:'inherit',boxShadow:'0 4px 12px rgba(22,163,74,.25)'}}>
              ...
              <div style={{display:'flex',gap:8}}>
                ...
                {selected.status==='suspended' && (
                  <button onClick={()=>activate(selected.id,selected.subscription_type,30)} disabled={!!saving}
                    style={{flex:1,padding:'11px',background:'#f0fdfa',color:'#029FA2',border:'1px solid #99f6e4',borderRadius:10,fontSize:13,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>
      ...
      <div style={{background:'#ffffff',borderBottom:'1px solid #e5e7eb',padding:'0 24px',display:'flex',alignItems:'center',justifyContent:'space-between',height:60,position:'sticky',top:0,zIndex:100,boxShadow:'0 1px 3px rgba(15,23,42,.03)'}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <div style={{width:36,height:36,background:'linear-gradient(135deg,#029FA2,#0f766e)',borderRadius:10,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18}}>🏪</div>
          <div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          {currentAdmin?.role==='super_admin' && (
            <a href="/storely-admin/audit-log" style={{padding:'7px 14px',background:'#ffffff',color:'#475569',border:'1px solid #e5e7eb',borderRadius:9,fontSize:12,fontWeight:700,textDecoration:'none',display:'flex',alignItems:'center',gap:6}}>
              📋 سجل التدقيق
            </a>
          )}
          {currentAdmin?.role==='super_admin' && (
            <a href="/storely-admin/demo-requests" style={{padding:'7px 14px',background:'#ffffff',color:'#475569',border:'1px solid #e5e7eb',borderRadius:9,fontSize:12,fontWeight:700,textDecoration:'none',display:'flex',alignItems:'center',gap:6}}>
              📩 طلبات العرض
            </a>
          )}
          {currentAdmin?.role==='super_admin' && (
            <a href="/storely-admin/partners" style={{padding:'7px 14px',background:'#ffffff',color:'#475569',border:'1px solid #e5e7eb',borderRadius:9,fontSize:12,fontWeight:700,textDecoration:'none',display:'flex',alignItems:'center',gap:6}}>
              🤝 الشركاء
            </a>
          )}
          {currentAdmin?.role==='super_admin' && (
          ...
          {currentAdmin?.role==='super_admin' && (
            <a href="/storely-admin/notification-health" style={{padding:'7px 14px',background:'#ffffff',color:'#475569',border:'1px solid #e5e7eb',borderRadius:9,fontSize:12,fontWeight:700,textDecoration:'none',display:'flex',alignItems:'center',gap:6}}>
              📶 صحة واتساب
            </a>
          )}
          ...
          {(currentAdmin?.role==='super_admin' || currentAdmin?.permissions?.manage_suppliers) && (
            <button onClick={()=>{setTab('suppliers');loadSupplierApps()}} style={{padding:'7px 14px',background:tab==='suppliers'?'#029FA2':'#ffffff',color:tab==='suppliers'?'white':'#475569',border:`1px solid ${tab==='suppliers'?'#029FA2':'#e5e7eb'}`,borderRadius:9,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',gap:6}}>
      ...
      <div style={{padding:'24px',maxWidth:1300,margin:'0 auto'}}>
          ...
          <div style={{background: maintenanceMode?'#fef2f2':'#ffffff', border:`1.5px solid ${maintenanceMode?'#fecaca':'#e5e7eb'}`, borderRadius:14, padding:'16px 18px', marginBottom:20}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:maintenanceMode?12:0,flexWrap:'wrap' as const,gap:10}}>
              ...
              <button onClick={()=>toggleMaintenance(!maintenanceMode)} disabled={maintenanceSaving}
                style={{padding:'8px 18px',background:maintenanceMode?'#029FA2':'#dc2626',color:'white',border:'none',borderRadius:9,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit',opacity:maintenanceSaving?.6:1}}>
            ...
            {maintenanceStatusMsg && <div style={{fontSize:11,color:'#029FA2',marginTop:8}}>{maintenanceStatusMsg}</div>}
          </div>
        )}
        ...
        <div style={{background:'#faf5ff',border:'1.5px solid #e9d5ff',borderRadius:14,padding:'16px 18px',marginBottom:20,display:'flex',alignItems:'center',gap:12,flexWrap:'wrap' as const}}>
          ...
          {waCapacityMsg && <div style={{fontSize:11,color:'#029FA2',width:'100%'}}>{waCapacityMsg}</div>}
        </div>
        ...
        <div className="stats-grid ru" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:24}}>
          {[
            ...
            {label:'مفعّلون',           value:stats.active, sub:'نشط الآن',    icon:'✅', color:'#029FA2', glow:'#f0fdfa'},
        ...
        <div className="ru" style={{background:'#ffffff',borderRadius:16,border:'1px solid #e5e7eb',overflow:'hidden'}}>
          {loading ? (
            <div style={{padding:64,textAlign:'center'}}>
              <div style={{width:36,height:36,border:'3px solid #e5e7eb',borderTopColor:'#029FA2',borderRadius:'50%',animation:'spin .7s linear infinite',margin:'0 auto 14px'}}/>
              ...
              <table style={{width:'100%',borderCollapse:'collapse',minWidth:700}}>
                ...
                <tbody>
                  {filtered.map(u=>{
                    ...
                    const planColor = u.max_branches===1?'#029FA2':u.max_branches<=3?'#2563eb':'#7c3aed'
                    return (
                      <tr key={u.id} className="urow" onClick={()=>setSelected(u)}>
                        <td style={{padding:'14px 16px'}}>
                          <div style={{display:'flex',alignItems:'center',gap:10}}>
                            <div style={{width:38,height:38,borderRadius:11,background:'linear-gradient(135deg,#029FA2,#0f766e)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,fontWeight:800,color:'white',flexShrink:0}}>
                              {u.full_name[0]||'؟'}
                            </div>
                            <div>
        ...
        <div style={{padding:'0 24px 24px',maxWidth:1300,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
            {[
              ...
              {label:'مستخدمون جدد هذا الشهر',value:dashStats.newThisMonth||0,icon:'🆕',color:'#029FA2'},
            ...
            <div style={{background:'#ffffff',borderRadius:14,padding:'20px',border:'1px solid #e5e7eb'}}>
              ...
              {users.slice(0,5).map((u,i)=>(
                <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'1px solid #f1f5f9'}}>
                  ...
                  <span style={{fontSize:11,color:'#029FA2',fontWeight:700}}>{u.subscription_type}</span>
                </div>
              ))}
            </div>
            <div style={{background:'#ffffff',borderRadius:14,padding:'20px',border:'1px solid #e5e7eb'}}>
              ...
              {[
                {label:'الأساسية',count:users.filter(u=>u.max_branches===1).length,color:'#029FA2'},
      ...
      {tab==='admins' && (
        <div style={{padding:'0 24px 24px',maxWidth:900,margin:'0 auto'}}>
          ...
          <div style={{background:'#ffffff',borderRadius:14,padding:20,border:'1px solid #e5e7eb',marginBottom:20}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:setup2FAOpen?16:0}}>
              <div>
                <div style={{fontSize:15,fontWeight:800,color:'#0f172a'}}>🔐 التحقق بخطوتين لحسابك</div>
                <div style={{fontSize:12,color:'#64748b',marginTop:2}}>
                  {(currentAdmin as any)?.totp_enabled ? 'مفعّل حالياً — حسابك محمي إضافياً' : 'غير مفعّل — يوصى بتفعيله لحماية إضافية'}
                </div>
              </div>
              {(currentAdmin as any)?.totp_enabled ? (
                ...
                <button onClick={start2FASetup} style={{padding:'8px 16px',background:'#029FA2',color:'white',border:'none',borderRadius:9,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>تفعيل الآن</button>
              )}
            </div>
            {setup2FAOpen && (
              <div style={{marginTop:16,paddingTop:16,borderTop:'1px solid #f1f5f9',textAlign:'center' as const}}>
                <p style={{fontSize:12,color:'#64748b',marginBottom:12}}>امسح الرمز بتطبيق المصادقة (Google Authenticator)، بعدها أدخل الرمز المكوّن من 6 أرقام:</p>
                {setup2FAQr && <img src={setup2FAQr} alt="QR" style={{width:180,height:180,margin:'0 auto 12px',display:'block',borderRadius:8,border:'1px solid #e5e7eb'}}/>}
                <input type="text" inputMode="numeric" maxLength={6} value={setup2FACode} onChange={e=>setSetup2FACode(e.target.value.replace(/\D/g,''))} dir="ltr"
                  style={{width:200,padding:'12px',border:'1.5px solid #e2e8f0',borderRadius:10,fontSize:20,textAlign:'center' as const,letterSpacing:8,outline:'none',fontFamily:'inherit',marginBottom:10}}
                  placeholder="000000"/>
                <br/>
                <button onClick={confirm2FASetup} disabled={setup2FALoading || setup2FACode.length!==6}
                  style={{padding:'10px 24px',background:'#029FA2',color:'white',border:'none',borderRadius:9,fontSize:13,fontWeight:700,cursor:'pointer',fontFamily:'inherit',opacity:setup2FACode.length!==6?.5:1}}>
                  {setup2FALoading?'⏳ جاري التأكيد...':'تأكيد وتفعيل'}
                </button>
                {setup2FAMsg && <div style={{marginTop:10,fontSize:12,fontWeight:700,color:setup2FAMsg.startsWith('✅')?'#029FA2':'#dc2626'}}>{setup2FAMsg}</div>}
              </div>
            )}
          </div>
          ...
          {showAddAdmin && (
            <div style={{background:'#ffffff',borderRadius:14,padding:20,border:'1px solid #e5e7eb',marginBottom:16}}>
              ...
              <button onClick={createAdmin} disabled={addAdminSaving}
                style={{width:'100%',padding:'11px',background:'#029FA2',color:'white',border:'none',borderRadius:9,fontSize:13,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div>
                      <div style={{display:'flex',alignItems:'center',gap:8}}>
                        ...
                        {a.role==='super_admin' && <span style={{fontSize:10,fontWeight:700,background:'#f0fdfa',color:'#029FA2',border:'1px solid #99f6e4',borderRadius:20,padding:'2px 8px'}}>مشرف كامل</span>}
                    ...
                    {a.role!=='super_admin' && (
                      <div style={{display:'flex',gap:6}}>
                        ...
                        <button onClick={()=>toggleAdminActive(a.id, !a.is_active)}
                          style={{padding:'6px 14px',background:a.is_active?'#fef2f2':'#f0fdfa',color:a.is_active?'#dc2626':'#029FA2',border:`1px solid ${a.is_active?'#fecaca':'#99f6e4'}`,borderRadius:8,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12}}>
                    <div style={{flex:1}}>
                      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                        ...
                        <span style={{padding:'2px 10px',borderRadius:99,fontSize:11,fontWeight:700,
                          ...
                          color:s.status==='pending'?'#d97706':s.status==='approved'?'#029FA2':'#dc2626'}}>
                    ...
                    <div style={{display:'flex',gap:6,flexShrink:0}}>
                      <a href={`https://wa.me/${s.phone.replace(/[^0-9]/g,'')}`} target="_blank" rel="noopener noreferrer"
                        style={{padding:'7px 12px',background:'#f0fdfa',color:'#029FA2',border:'1px solid #99f6e4',borderRadius:8,fontSize:12,fontWeight:700,textDecoration:'none'}}>
                        📲 واتساب
                      </a>
                      {s.status==='pending'&&<>
                        <button onClick={()=>updateSupplierStatus(s.id,'approved')}
                          style={{padding:'7px 12px',background:'#f0fdfa',color:'#029FA2',border:'1px solid #99f6e4',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>
                          ✅ موافقة
                        </button>
