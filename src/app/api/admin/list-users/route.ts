...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabase
    .from('profiles')
    .select('id,full_name,phone,role,status,created_at,org_id,subscription_type,subscription_ends_at,branch_count,organizations(name,max_branches,billing_cycle,plan,requested_plan)')
