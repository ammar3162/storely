...
// إلغاء التحقق الثنائي — يتطلب رمز صحيح حالي كإجراء حماية إضافي
export async function POST(req: Request) {
  try {
    ...
    if (!(admin as any)?.totp_secret) return NextResponse.json({ error: 'غير مفعّل أصلاً' }, { status: 400 })
    ...
    await (db as any).from('admin_users').update({ totp_enabled: false, totp_secret: null }).eq('id', session.id)
