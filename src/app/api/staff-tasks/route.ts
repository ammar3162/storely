...
export async function GET(req: Request) {
  try {
    ...
    let q = supabase.from('staff_tasks').select('*,staff_members(name)').eq('org_id', orgId).order('created_at', { ascending: false })
...
// الموظف يكمّل مهمة (بتوكنه)، أو المالك يؤكد/يرفض إتمام مهمة (بـorg_id)
export async function PATCH(req: Request) {
  try {
    ...
    if (body.org_id) {
      // المالك يؤكد/يرفض
      const { org_id, task_id, decision } = body
