...
export async function POST(req: Request) {
  try {
    const { branch_id, org_id, user_id } = await req.json()
    if (!branch_id || !org_id || !user_id) {
      return NextResponse.json({ error: 'بيانات ناقصة' }, { status: 400 })
    }

    const db = sb()

    // تحقق إن المستخدم فعلاً صاحب هذي المؤسسة (يمنع أي محاولة تلاعب بالطلب)
    const { data: profile } = await db.from('profiles').select('org_id').eq('id', user_id).maybeSingle()
