...
const STATUS_INFO: Record<string,{label:string;color:string;bg:string}> = {
  ...
  confirmed: { label:'مؤكد',        color:'#029FA2', bg:'#f0fdfa' },
    ...
    <div style={{minHeight:'100vh',fontFamily:"'IBM Plex Sans Arabic',system-ui",direction:'rtl',background:'#faf8f5',color:'#1c1917'}}>
      {screen === 'hero' ? (
        <div style={{minHeight:'100vh',display:'flex',flexDirection:'column' as const,alignItems:'center',justifyContent:'center',padding:24,textAlign:'center' as const}}>
          ...
          {status.known && (
            <span style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:11,fontWeight:700,padding:'4px 12px',borderRadius:99,background:status.open?'#f0fdfa':'#fef2f2',color:status.open?'#029FA2':'#dc2626',marginBottom:20}}>
              <span style={{width:6,height:6,borderRadius:'50%',background:status.open?'#029FA2':'#dc2626'}}/>{status.label}
            </span>
          )}
