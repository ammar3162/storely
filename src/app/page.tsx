...
const BUSINESSES = [
  ...
  { icon: '💊', label: 'صيدليات' },
  { icon: '🏭', label: 'مستودعات' },
  { icon: '🛍️', label: 'متاجر إلكترونية' },
]

const FEATURES = [
  { icon: '📦', title: 'تتبع المخزون لحظة بلحظة', desc: 'راقب كميات منتجاتك بدقة واستقبل تنبيهات فورية قبل أن ينفد أي صنف', color: '#16a34a' },
  { icon: '📲', title: 'تنبيهات واتساب تلقائية', desc: 'يصلك إشعار فوري على واتساب لما يوصل أي صنف للحد الأدنى — بدون ما تراقب يدوياً', color: '#25d366' },
...
const PROBLEMS = [
  { problem: 'لا تعرف وش نفد من مخزونك إلا بعد ما ينتهي', solution: 'تنبيهات فورية قبل النفاد' },
  { problem: 'تسجيل المخزون يدوياً في ورق أو Excel', solution: 'نظام رقمي احترافي سريع' },
...
const STEPS = [
  ...
  { n: '02', title: 'أضف مخزونك', desc: 'أدخل منتجاتك يدوياً أو اختر من القوالب الجاهزة لنشاطك' },
...
const PLANS = [
  { name: 'الأساسية', price: '149', branches: 'فرع واحد', color: '#16a34a', features: ['تتبع المخزون', 'تنبيهات واتساب', 'موظف واحد', 'التقارير', 'نسخ احتياطي'] },
  { name: 'المتوسطة', price: '249', branches: '2-3 فروع', color: '#2563eb', popular: true, features: ['كل مميزات الأساسية', 'حتى 3 فروع', 'موظفون غير محدودين', 'تقارير موسعة', 'دعم أولوي'] },
  { name: 'المتقدمة', price: '399', branches: '4+ فروع', color: '#7c3aed', features: ['كل مميزات المتوسطة', 'فروع غير محدودة', 'تقارير مخصصة', 'دعم مخصص 24/7'] },
]
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: '1px solid #f1f5f9', marginBottom: 0 }}>
      ...
      {open && (
        <div style={{ paddingBottom: 22, fontSize: 15, color: '#64748b', lineHeight: 1.8 }}>{a}</div>
      )}
    </div>
  )
}
    ...
    <div style={{ fontFamily: "'IBM Plex Sans Arabic',system-ui,sans-serif", direction: 'rtl', background: '#ffffff', overflowX: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .step-card { background: white; border: 1.5px solid #f1f5f9; border-radius: 20px; padding: 28px 24px; position: relative; transition: all .3s; }
        ...
        .btn-outline { background: transparent; color: #16a34a; border: 2px solid #16a34a; border-radius: 14px; padding: 14px 28px; font-size: 15px; font-weight: 700; cursor: pointer; font-family: inherit; transition: all .25s; }
      ...
      <nav style={{ position: 'fixed', top: 0, right: 0, left: 0, zIndex: 1000, padding: '14px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: scrolled ? 'rgba(255,255,255,.97)' : 'transparent', backdropFilter: scrolled ? 'blur(12px)' : 'none', borderBottom: scrolled ? '1px solid #f1f5f9' : 'none', transition: 'all .3s', boxShadow: scrolled ? '0 2px 20px rgba(0,0,0,.06)' : 'none' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => router.push('/login')} style={{ background: scrolled ? '#f1f5f9' : 'rgba(255,255,255,.12)', color: scrolled ? '#1e293b' : 'white', border: 'none', borderRadius: 10, padding: '10px 20px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'all .2s' }}>دخول</button>
      ...
      <section style={{ minHeight: '100vh', background: 'linear-gradient(160deg,#0a1f13 0%,#0d2818 50%,#0a1a0d 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '120px 24px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 820 }}>
          ...
          <p style={{ fontSize: 18, color: 'rgba(255,255,255,.65)', maxWidth: 560, margin: '0 auto 36px', lineHeight: 1.8 }}>
            منصة عربية ذكية تتابع مخزونك، تنبّهك قبل النفاد، وتخلّي موظفيك يصرفون بضغطة واحدة
          </p>
          ...
          <div className="hero-btns" style={{ display: 'flex', gap: 14, justifyContent: 'center' }}>
            <button onClick={() => router.push('/login')} className="btn-primary" style={{ fontSize: 17, padding: '16px 36px' }}>
              ابدأ مجاناً الآن ←
            </button>
            <a href="#features" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '16px 28px', border: '1.5px solid rgba(255,255,255,.2)', borderRadius: 14, color: 'rgba(255,255,255,.8)', textDecoration: 'none', fontSize: 15, fontWeight: 600, transition: 'all .2s', background: 'rgba(255,255,255,.05)' }}>
              اكتشف المميزات
            </a>
          </div>
          ...
          <div style={{ display: 'flex', gap: 48, justifyContent: 'center', marginTop: 64, flexWrap: 'wrap' }}>
            {[['99 ر.س', 'يبدأ من'], ['7', 'لغات مدعومة'], ['24/7', 'تنبيهات تلقائية']].map(([n, l]) => (
      ...
      <section style={{ padding: '100px 24px', background: '#f8fafc' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div id="prob-title" data-animate style={{ textAlign: 'center', marginBottom: 56 }} className={"fade-up" + (isVis("prob-title") ? " visible" : "")}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#16a34a', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 12 }}>المشاكل التي نحلها</div>
          ...
          <div className="prob-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {PROBLEMS.map((p, i) => (
              <div key={i} id={"prob-" + i} data-animate className={`prob-row fade-up${isVis("prob-" + i) ? ' visible' : ''}`} style={{ transitionDelay: (i * 0.1) + 's' }}>
                <div style={{ flexShrink: 0 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: '#fee2e2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>❌</div>
                </div>
                <div style={{ flex: 1 }}>
      ...
      <section id="features" style={{ padding: '100px 24px', background: 'white' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div id="feat-title" data-animate style={{ textAlign: 'center', marginBottom: 64 }} className={"fade-up" + (isVis("feat-title") ? " visible" : "")}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#16a34a', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 12 }}>المميزات</div>
          ...
          <div className="feat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
            {FEATURES.map((f, i) => (
              <div key={i} id={"feat-" + i} data-animate className={`feat-card fade-up${isVis("feat-" + i) ? ' visible' : ''}`} style={{ transitionDelay: (i * 0.08) + 's' }}>
      ...
      <section id="how" style={{ padding: '100px 24px', background: 'linear-gradient(160deg,#0a1f13,#0d2818)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div id="how-title" data-animate style={{ textAlign: 'center', marginBottom: 64 }} className={"fade-up" + (isVis("how-title") ? " visible" : "")}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#4ade80', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 12 }}>كيف يعمل</div>
          ...
          <div className="step-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
            {STEPS.map((s, i) => (
              <div key={i} id={"step-" + i} data-animate className={`step-card fade-up${isVis("step-" + i) ? ' visible' : ''}`} style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', transitionDelay: (i * 0.1) + 's' }}>
                <div style={{ fontSize: 32, fontWeight: 900, color: '#4ade80', marginBottom: 14, fontVariantNumeric: 'tabular-nums' }}>{s.n}</div>
      ...
      <section style={{ padding: '100px 24px', background: '#f8fafc' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
          <div id="who-title" data-animate style={{ marginBottom: 52 }} className={"fade-up" + (isVis("who-title") ? " visible" : "")}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#16a34a', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 12 }}>من نخدم</div>
            <h2 style={{ fontSize: 38, fontWeight: 900, color: '#0f172a', marginBottom: 12 }}>مناسب لكل نشاط تجاري</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
            {BUSINESSES.map((b, i) => (
              <div key={i} id={"biz-" + i} data-animate className={"fade-up" + (isVis("biz-" + i) ? " visible" : "")} style={{ background: 'white', border: '1.5px solid #f1f5f9', borderRadius: 18, padding: '24px 16px', textAlign: 'center', transition: 'all .25s', transitionDelay: (i * 0.06) + 's', cursor: 'default' }}
      ...
      <section id="pricing" style={{ padding: '100px 24px', background: 'white' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div id="price-title" data-animate style={{ textAlign: 'center', marginBottom: 64 }} className={"fade-up" + (isVis("price-title") ? " visible" : "")}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#16a34a', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 12 }}>الأسعار</div>
          ...
          <div className="plan-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
            {PLANS.map((p, i) => (
              <div key={i} id={"plan-" + i} data-animate className={"plan-card fade-up" + (isVis("plan-" + i) ? " visible" : "")}
                style={{ background: p.popular ? 'linear-gradient(160deg,#0a1f13,#0d2818)' : '#f8fafc', border: p.popular ? '2px solid #16a34a' : '1.5px solid #e2e8f0', position: 'relative', transitionDelay: (i * 0.1) + 's' }}>
                {p.popular && <div style={{ position: 'absolute', top: -14, right: '50%', transform: 'translateX(50%)', background: '#16a34a', color: 'white', padding: '5px 20px', borderRadius: 99, fontSize: 12, fontWeight: 700 }}>الأكثر طلباً</div>}
                ...
                <button onClick={() => router.push('/login')}
                  style={{ width: '100%', padding: '14px', background: p.popular ? '#16a34a' : 'white', color: p.popular ? 'white' : '#16a34a', border: p.popular ? 'none' : '2px solid #16a34a', borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'all .2s', boxShadow: p.popular ? '0 6px 20px rgba(22,163,74,.4)' : 'none' }}>
                  ابدأ الآن
                </button>
              </div>
            ))}
      ...
      <section style={{ padding: '100px 24px', background: 'linear-gradient(160deg,#0a1f13,#0d2818)', textAlign: 'center' }}>
        <div id="cta" data-animate style={{ maxWidth: 600, margin: '0 auto' }} className={"fade-up" + (isVis("cta") ? " visible" : "")}>
          ...
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => router.push('/login')} className="btn-primary" style={{ fontSize: 17, padding: '17px 40px' }}>
              سجّل مجاناً الآن ←
            </button>
      ...
      <section style={{ padding: '100px 24px', background: 'white' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          {[
            ...
            { q: 'هل يدعم متعدد الفروع؟', a: 'نعم — باقة 199 ر.س تدعم حتى 3 فروع، وباقة 349 ر.س تدعم فروع غير محدودة.' },
            { q: 'هل الموظفون يحتاجون تدريب؟', a: 'لا — واجهة الموظفين بسيطة جداً بضغطة واحدة للصرف، وتدعم 7 لغات لأي جنسية.' },
            { q: 'هل بياناتي آمنة؟', a: 'نعم — بياناتك محفوظة على سيرفرات مشفرة مع نسخ احتياطية أسبوعية تلقائية.' },
